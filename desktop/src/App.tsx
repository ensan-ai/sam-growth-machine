...
import type { Activity, Approval, Dashboard, Employee, NovaResearchReport, Settings, WorkDetail, WorkItem } from "./types";
...
export default function App(){
  ...
  return <div className={`app-shell view-${view.toLowerCase()}`}>
    ...
    <main>
      ...
      {view==="TEAM"&&<EmployeeTeam team={team} activity={activity} onNovaResearch={()=>setNovaOpen(true)}/>} 
    ...
    {novaOpen&&<NovaResearchModal close={()=>setNovaOpen(false)} onComplete={refresh}/>} 
  </div>;
}
...
function NovaResearchModal({close,onComplete}:{close:()=>void;onComplete:()=>Promise<void>}){
  const [username,setUsername]=useState("@steventawfik");
  const [limit,setLimit]=useState(20);
  const [running,setRunning]=useState(false);
  const [failure,setFailure]=useState<string>();
  const [result,setResult]=useState<NovaResearchReport>();
  const run=async()=>{setRunning(true);setFailure(undefined);try{const report=await runtime.novaResearchCreator(username,limit);setResult(report);await onComplete();}catch(e){setFailure(String(e));}finally{setRunning(false);}};
  return <div className="modal-backdrop"><div className="modal glass"><button type="button" className="modal-close" onClick={close}><X/></button><span className="eyebrow">NOVA · CREATOR RESEARCH</span><h2>{result?result.creator:"Research Instagram creator"}</h2>{result?<><State state={result.status}/><p>Existing videos: <strong>{result.existingVideos}</strong></p><p>New videos added: <strong>{result.newVideosAdded}</strong></p><p>Total scripts stored: <strong>{result.totalScriptsStored}</strong></p><p>Known Video IDs skipped: <strong>{result.knownVideoIdsSkipped}</strong></p><p>Metrics refreshed: <strong>{result.metricsRefreshed}</strong></p><p>Transcript failures: <strong>{result.transcriptionFailures}</strong></p>{result.relevantVideos.length?<div><span className="eyebrow">Relevant videos</span>{result.relevantVideos.map(v=><p key={v.videoId}><a href={v.reelUrl} target="_blank" rel="noreferrer">Open Reel</a> · {v.reason}</p>)}</div>:null}<div className="form-actions"><a href={result.notion.scriptsDatabaseUrl} target="_blank" rel="noreferrer">Open scripts in Notion</a><button className="primary" onClick={()=>setResult(undefined)}>Research another</button></div></>:<><label>Instagram @ID<input value={username} onChange={e=>setUsername(e.target.value)} placeholder="@creator"/></label><label>Maximum Reels<input type="number" min={1} max={100} value={limit} onChange={e=>setLimit(Math.max(1,Math.min(100,Number(e.target.value)||20)))}/></label><p>NOVA checks Notion first, skips known Video IDs, and processes only new Reels.</p>{failure&&<p className="run-error">{failure}</p>}<div className="form-actions"><button onClick={close}>Cancel</button><button className="primary" disabled={running||!username.trim()} onClick={run}>{running?"Researching…":"Run NOVA"}</button></div></>}</div></div>;
}
