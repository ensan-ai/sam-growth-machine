...
export function MissionHome({dashboard,team,activity,handoffs,selectEmployee,selectWork,openActivity,openApprovals}:Props){
 ...
 const signals=newestFirst(activity.events.filter(e=>e.eventType!=="agent.started"),e=>e.createdAt).slice(0,5);
 return <section className="mission-home">
  <div className="mission-body">
   ...
   <aside className="signal-rail"><div className="rail-heading"><h2>Latest signals</h2><GitBranch size={16}/></div><p className="rail-subtitle">From your company, in real time</p><Feed className="signal-items" pinKey={`signals:${signals[0]?.eventId||""}:${signals.length}`}>{signals.length?signals.map(e=><button className="signal-item" key={e.eventId} onClick={()=>e.workItemId?selectWork(e.workItemId):openActivity()}><div><span className={`signal-dot ${tone(e)}`}/><span className="actor">{e.actor}</span><time>{time(e.createdAt)}</time></div><strong>{eventLabel(e.eventType)}</strong><p>{signalDescription(e)}</p></button>):<p className="quiet-note">The signal stream is quiet.<br/>New activity will appear here.</p>}</Feed><button className="text-link" onClick={openActivity}>View all activity <ArrowRight size={15}/></button></aside>
  </div>
